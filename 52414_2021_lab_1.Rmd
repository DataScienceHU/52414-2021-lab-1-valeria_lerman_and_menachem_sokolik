...
data <- full_join(dat1,dat2,by = "location")
```
###The scatter plot
```{r}
...
joindata <- full_join(col1,col2,by = "location")
...
```{r}
hist(tempdata$death_rate, breaks=50, main="death rate frequency",xlab = "death rate",col='#65BDED', border="white", font.axis=2)
```
...
cross_correlation <- function(data_frame,country,column1,column2){
  ...
  negative <- seq(60,0,-1)
  Positive <- seq(1:60)
  required.location[is.na(required.location)] <- 0
  cross_correlation=c()
  for (i in negative){
    ...
    cross_correlation=c(cross_correlation, cor(y_t, x_t))}
  for (i in Positive){
    delta_t_x <- seq(min_date, max_date-i, by="days")
    delta_t_y <- seq(min_date+i, max_date, by="days")
    x_t <- required.location %>% filter(date %in% delta_t_x) %>% select(column1)
    y_t <- required.location %>% filter(date %in% delta_t_y) %>% select(column2)
    cross_correlation = c(cross_correlation, cor(x_t,y_t))}
