...
The meaning of this fact, is that if someone gets infected by the virus on a specific date, then he would likely die approximately 17 days afterwards.
...
```{r}
with(sub.data, plot(y=log(total_vaccinations_per_hundred), x=log(ratio_between_current_maximal), main='comparing the vaccination rate and the ratio rate', cex=.9, col='#0078FF', ylab='log total vaccinations per hundred', xlab='log ratio between current dna maximal rate'))
par(new=TRUE)
with(outliers, points(log(ratio_between_current_maximal), log(total_vaccinations_per_hundred), cex=.9, col='red'))
with(outliers, text(log(total_vaccinations_per_hundred)~log(ratio_between_current_maximal), labels=location, cex=0.7, pos=2, col='red'))
```
