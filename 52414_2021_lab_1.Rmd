...
The slope is equal to `r coef(reg)[2]`, which means that for every case of the virus, there will be `r coef(reg)[2]` death. In other words, for every 1000 cases, there will be approximately `r coef(reg)[2]*1000` death cases.
Note that the intercept we got from the regression summary is around `r coef(reg)[1]`, which doesn't seem possible, as it represents the number of total deaths per million for zero total cases per million.
...
The x-axis represents the dates of max new cases for each country, so in our opinion the intercept represents the date of max new deaths for the first country in our data frame.The intercept in our regression is equal to  `r coef(reg)[1]` , which is not very informative because the y axis represents dates, and not numbers, whereas the regression interprets the given dates as numeric. 
...
```
We can see according to the graph that the peak appears around the 17 day.
