...
### The data frame
```{r}
newmat=data.frame(c(df$location),c(df$total_cases_per_million),c(df$total_deaths_per_million))
newmat[is.na(newmat)] <- 0
newmat$c.df.total_cases_per_million.=as.numeric(newmat$c.df.total_cases_per_million.)
newmat$c.df.total_deaths_per_million.=as.numeric(newmat$c.df.total_deaths_per_million.)
agg= aggregate(newmat[-1], by = list(newmat$c.df.location), FUN = sum)
colnames(agg)=c("location","total_cases_per_million","total_deaths_per_million")
head(agg)
```
...
```{r}
options(scipen = 100)
plot(agg$total_cases_per_million, agg$total_deaths_per_million, main="Scatterplot of the number of deaths per million as a function of the number of cases per million",
   xlab="total_cases_per_million ", ylab="total_deaths_per_million ", pch=19)
abline(lm(agg$total_deaths_per_million~agg$total_cases_per_million), col="red")
reg=lm(agg$total_deaths_per_million~agg$total_cases_per_million)
summary(reg)
```
...
```{r}
newmat2=data.frame(c(df$location),c(df$new_cases),c(df$new_deaths))
newmat2[is.na(newmat2)] <- 0
newmat2$c.df.new_cases.=as.numeric(newmat2$c.df.new_cases.)
newmat2$c.df.new_deaths.=as.numeric(newmat2$c.df.new_deaths.)
agg2= aggregate(newmat2[-1], by = list(newmat2$c.df.location), FUN = max)
colnames(agg2)=c("location","max_new_cases","max_new_deaths")
finaldataframe=cbind(agg,agg2[,2:3])



options(scipen = 100)
plot(finaldataframe$max_new_cases, finaldataframe$max_new_deaths, main="Scatterplot of the max number of new deaths as a function of the max number of new cases",
   xlab="max_new_cases ", ylab="max_new_deaths ", pch=19)
abline(lm(finaldataframe$max_new_deaths~finaldataframe$max_new_cases), col="red")
reg2=lm(finaldataframe$max_new_deaths~finaldataframe$max_new_cases)
summary(reg2)
```
